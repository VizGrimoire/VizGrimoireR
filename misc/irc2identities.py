...
def getOptions():     
    parser = OptionParser(usage='Usage: %prog [options]', 
                          description='IRC identities detection using nicks',
                          version='0.1')    
    parser.add_option('--db-database-irc', dest='db_database_irc',
...
def main():
   ...
   db_irc, connector_irc = connect(cfg.db_database_irc, cfg)
   db_ids, connector_ids = connect(cfg.db_database_ids, cfg)

   create_tables(db_irc, connector_irc)

   query = "select DISTINCT(nick) from irclog"
   results = execute_query(connector_irc, query)
   total = len(results)
   newids = 0
   reusedids = 0
   done = 0
   ...
   for result in results:
      ...
      if len(results_ids) > 0:
         ...
         execute_query(connector_irc, query)
      else:
         newids = newids+1
